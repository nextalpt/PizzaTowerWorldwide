...
{
	if ((global.panic or global.snickchallenge == true && global.minutes < 1) || player.sprite_index == spr_player_timesup)
	{
		shake_mag = 2;
		shake_mag_acc = 3 / room_speed;
	}
	else if (global.panic or global.snickchallenge == true && basement == 0)
	{
		shake_mag = 2;
		shake_mag_acc = 3 / room_speed;
	}
		...
		case states.normal:
			...
			if (targetgolf == -4)
			{
				if (!global.coop || room == characterselect || room == rm_levelselect || room == Realtitlescreen)
				{
					...
					{
						cam_x += (((((target.x - (cam_width / 2)) + chargecamera) + p2pdistancex) - cam_x) * 0.25)
